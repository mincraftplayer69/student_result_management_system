...
<body>
  ...
  <a href="section.php"><img class="back-btn" src="images/left-arrow.png"></a>
    <h1 class="inner-heading">Section Management <img class="back-btn" style="position: relative;" src="images/right-arrow.png"> STEM</h1>
  <main>
    <div class="tables">
      <label>Select Strand:</label>
      <a href="ict.php"><button style=" margin-left: 1%; font-size: 10px; font-family: 'Public Sans', sans-serif;padding: 5px 10px; border-radius: 30px; background-color: #f4f4f4;">ICT</button></a>
      ...
      <a href="gas.php"><button style=" margin-left: 2%; font-size: 10px; font-family: 'Public Sans', sans-serif;padding: 5px 10px; border-radius: 30px; background-color: #f4f4f4;">GAS</button></a>
  ...
  <table id = "table" class = "table-bordered">
          ...
          <tbody>
            ...
            include_once "database/config.php";
              $query = $conn->query("SELECT * FROM `users` WHERE stdnt_strand='STEM' AND stdnt_section='1A'") or die(mysqli_error());
          ...
          <tbody>
            <?php
            include_once "database/config.php";
              $query = $conn->query("SELECT * FROM `users` WHERE stdnt_strand='STEM' AND stdnt_section='2A'") or die(mysqli_error());
              while($f_query = $query->fetch_array()){
            ?>
            <tr>
              <td><?php echo $f_query['lastName']?></td>
          ...
          <tbody>
            <?php
            include_once "database/config.php";
              $query = $conn->query("SELECT * FROM `users` WHERE stdnt_strand='STEM' AND stdnt_section='1P'") or die(mysqli_error());
              while($f_query = $query->fetch_array()){
            ?>
            <tr>
              <td><?php echo $f_query['lastName']?></td>
